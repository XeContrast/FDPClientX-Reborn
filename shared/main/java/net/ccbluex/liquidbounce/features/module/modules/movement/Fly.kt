...
class Fly : Module() {
    ...
    private val redeskyTimerValue = FloatValue("Redesky-TimerSpeed", 1f, 0.2f, 2f)
    private val redeskyYValue = FloatValue("Redesky-BoostY", 4F, 2F, 10F)
    private val redeskyTicksExistedValue = IntegerValue("Redesky-TicksToWait", 8, 2, 20)
    ...
    override fun onEnable() {
        ...
        run {
            when (mode.toLowerCase()) {
                ...
                "redesky" -> {
                    mc.timer.timerSpeed = redeskyTimerValue.get()
                }
                "boosthypixel" -> {
                    if (!thePlayer.onGround)
                        return@run
    ...
    fun onUpdate(event: UpdateEvent?) {
        ...
        run {
            when (modeValue.get().toLowerCase()) {
                ...
                "redesky" -> {
                            var yaw = toRadians(mc.thePlayer!!.rotationYaw.toDouble())
                            var mx = mc.thePlayer!!.posX - (sin(yaw) * 7)
                            var mz = mc.thePlayer!!.posZ + (cos(yaw) * 7)
                    if(mc.thePlayer!!.ticksExisted % redeskyTicksExistedValue.get() == 0)
                        mc.netHandler.addToSendQueue(classProvider.createCPacketPlayerPosition(mx, mc.thePlayer!!.posY + redeskyYValue.get(), mz, true));
                }
                "minesucht" -> {
                ...
                "jetpack" -> if (mc.gameSettings.keyBindJump.isKeyDown) {
                    mc2.effectRenderer.spawnEffectParticle(EnumParticleTypes.FLAME.getParticleID(), thePlayer.posX, thePlayer.posY + 0.2, thePlayer.posZ, -thePlayer.motionX, -0.5, -thePlayer.motionZ)
