...
//	public ScrollingText artistsScroll;
    ...
    public MusicManager() {
        ...
        circleImage = new File(Minecraft.getMinecraft().mcDataDir.toString() + File.separator + FDPClient.CLIENT_NAME + File.separator + "circleImage");
        ...
        if (!mp3File.exists() && !flacFile.exists()) {
            ...
            loadingThread = new Thread(() -> {
                try {
                    ...
                    ClientUtils.INSTANCE.displayChatMessage("缓存音乐时发生错误, 可能是因为该歌曲已被下架或需要VIP!");
                    if (mp3File.exists())
                        mp3File.delete();
