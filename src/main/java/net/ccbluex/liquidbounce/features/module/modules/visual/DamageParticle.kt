        ...
        synchronized(particles) {
            for (entity in mc.theWorld.loadedEntityList) {
                if (entity is EntityLivingBase && EntityUtils.isSelected(entity, true)) {
                    ...
                    val colourPrefix = if (colourValue.get() == "Damage") (if (lastHealth > entity.health) {
                        "§c"
                    } else {
                        "§a"
                    }) else ""
                    val prefix = if (colourValue.get() != "Rainbow") (if (lastHealth > entity.health) {
                        "❤"
                    } else {
                        "❤"
                    ...
                    particles.add(
                        SingleParticle(
                            colourPrefix + prefix + BigDecimal(abs(lastHealth - entity.health).toDouble()).setScale(
                                1,
                            ...
                            entity.posZ - 0.5 + Random(System.currentTimeMillis() + 1L).nextInt(5)
                                .toDouble() * 0.1
                        )
                    )
                }
            ...
            for (particle in particles) {
                ...
                if (colourValue.get() != "Damage") {
                    when (shadowValue.get()) {
                        "LB" -> {
                            mc.fontRendererObj.drawString(particle.str, (x + 1), (y + 1), Color(0, 0, 0, 150).rgb)
                        }

                        "Autumn" -> {
                            mc.fontRendererObj.drawString(particle.str, (x + 1), (y + 1), Color(20, 20, 20, 150).rgb)
                        }
                ...
                mc.fontRendererObj.drawString(
                    ...
                    (if (colourValue.get() == "Rainbow") ColorUtils.rainbowWithAlpha(colorAlphaValue.get()) else Color(
                        colorRedValue.get(),
                        colorGreenValue.get(),
                        colorBlueValue.get(),
                        colorAlphaValue.get()
