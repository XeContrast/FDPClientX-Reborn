...
object AntiBot : Module() {
    ...
    fun isBot(entity: IEntityLivingBase): Boolean {
        ...
        if (duplicateInWorldValue.get() &&
            mc.theWorld!!.loadedEntityList.count { classProvider.isEntityPlayer(it) && it.asEntityPlayer().displayNameString == it.asEntityPlayer().displayNameString } > 1)
            return true

        if (duplicateInTabValue.get() &&
            mc.netHandler.playerInfoMap.
            count { entity.name == stripColor(EntityUtils.getName(it)) } > 1)
            return true
