...
object ChinaHat : Module() {
    ...
    private fun drawChinaHatFor(entity: EntityLivingBase) {
        ...
        for(i in 0..360 step 5) {
            if(colorRainbowValue.get()) {
                RenderUtils.glColor(Color.getHSBColor(if (i <180) { HUDModule.rainbowStartValue.get() + (HUDModule.rainbowStopValue.get() - HUDModule.rainbowStartValue.get()) * (i / 180f) } else { HUDModule.rainbowStartValue.get() + (HUDModule.rainbowStopValue.get() - HUDModule.rainbowStartValue.get()) * (-(i-360) / 180f) }, 0.7f, 1.0f), colorAlphaValue.get() / 255f)
            }
