...
object StorageESP : Module() {
    ...
    private fun getColor(tileEntity: TileEntity): Color? {
        if (chestValue.get() && tileEntity is TileEntityChest && !clickedBlocks.contains(tileEntity.getPos())) return Color(0, 0, 255)
        if (chestValue.get() && tileEntity is TileEntityChest && clickedBlocks.contains(tileEntity.getPos())) return Color(255, 0, 0)
