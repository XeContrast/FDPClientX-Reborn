...
class Arraylist(
    ...
    private val shadowShaderValue = BoolValue("Glow", true)
    private val shadowNoCutValue = BoolValue("Glow-NoCut", false)
    private val shadowStrength = IntegerValue("Glow-Strength", 1, 1, 30)
    private val shadowColorMode = ListValue("Glow-Color", arrayOf("Background", "Text", "Custom"), "Background")
    private val shadowColorRedValue = IntegerValue("Glow-Red", 0, 0, 255)
    private val shadowColorGreenValue = IntegerValue("Glow-Green", 111, 0, 255)
    private val shadowColorBlueValue = IntegerValue("Glow-Blue", 255, 0, 255)
    ...
    val textSpacer = textHeightValue.get() + spaceValue.get()
                    ...
                    if (!rectMode.equals("none", true)) {
                        ...
                        when (rectMode.lowercase()) {
                            "left" -> RenderUtils.drawRect(xPos - 5, yPos, xPos - 2, yPos + textHeight,
                                rectColor)
                            "right" -> RenderUtils.drawRect(-3F, yPos, -1F,
